...
#if UNITY_2023
        public static T Instance => instance ??= GameObject.FindFirstObjectByType<T>() ?? new GameObject(typeof(T).ToString()).AddComponent<T>();
#else
        public static T Instance => instance ??= FindObjectOfType<T>() ?? new GameObject(typeof(T).ToString()).AddComponent<T>();
#endif
