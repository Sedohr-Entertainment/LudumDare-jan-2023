...
namespace Ru1t3rl.Events
{
    public class EventManager : UnitySingleton<EventManager>
    {
        ...
        public void RemoveEvent(string key)
        {
            ...
            {
                events.Remove(events.GetEvent(key));
            }
            else if (eventsWithArgs.ContainsKey(key))
            {
                eventsWithArgs.Remove(eventsWithArgs.GetEvent(key));
            }
        }
        ...
        public void Invoke(UnityEvent unityEvent)
        {
            if (events.ContainsEvent(unityEvent))
            {
                events.GetEvent(unityEvent).Invoke();
            }
        }

        public void Invoke(UnityEvent<EventArgs> unityEvent, EventArgs args)
        {
            if (eventsWithArgs.ContainsEvent(unityEvent))
            {
                eventsWithArgs.GetEvent(unityEvent).Invoke(args);
            }
        }
    }
}